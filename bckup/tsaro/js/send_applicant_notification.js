...
jQuery('.textarea').wysihtml5();
 ...
 jQuery("#reply_notification").click(function() {
 	jQuery.validator.setDefaults({
		submitHandler: function() { 
			show_modal();
			jQuery.ajax({
			  type: "POST",
			  url: "ajax/ajax_reply_applicant_notification.php",
			  data: jQuery(".reply_notification").serialize()
			}).done(function( msg ) {
				display_modal(msg);
			});
		}
	 }); 
	 jQuery('.reply_notification').validate(
	 {
	  rules: {
		replymessage: {
	      required: true,
