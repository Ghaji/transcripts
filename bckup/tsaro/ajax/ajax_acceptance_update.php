<?php
	...
	$updatesql = "UPDATE acceptance_log SET PaymentReference='$payment_reference', Amount='$amount', returned_amount='$returned_amount', ResponseCode='$responsecode', ResponseDescription='$responseDescription', status='$status' WHERE id='$id'";
	...
	if($result){
		...
		echo '<hr>';
		echo "You have successfully edited the payment record for applicant with application number: ".$application_number;
		echo '<hr>';
	}else{
