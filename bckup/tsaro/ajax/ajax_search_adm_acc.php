...
//$query_adm_access = "SELECT * FROM acceptance_adm_access_code adm, personal_details p, department d, admission_status ads WHERE adm.jamb_rem_no = p.form_id AND p.programme_applied_id = d.department_id AND p.applicant_id = ads.applicant_id". $whereclause;

$query_adm_access = "SELECT * FROM acceptance_adm_access_code adm, personal_details p, department d, faculty f WHERE adm.jamb_rem_no = p.form_id AND p.programme_applied_id = d.department_id AND f.faculty_id = d.faculty_id". $whereclause;
...
if($number_of_records > 0)
{
	...
	while ($record = $database->fetch_array($result)){ 
		...
		<tr>';
			...
			<td><a href="view_payment_acc.php?q='. customEncrypt($record["jamb_rem_no"]) .'" class="btn">View</a></td>
		</tr>';
