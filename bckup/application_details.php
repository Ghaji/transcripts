<?php
    ...
    if($label) {
        ...
        $sql_app  = "SELECT * FROM `app_histories` WHERE `application_no` = '".$label."' LIMIT 1";
        $application= AppHistory::find_by_sql($sql_app);
        $myapp= array_shift($application);

        $sql_amount  = "SELECT * FROM `adm_access_code` WHERE `jamb_rem_no` = '".$label."' LIMIT 1";
        $transaction=AdmAccess::find_by_sql($sql_amount);
        $amount= array_shift($transaction);


        $sql = "SELECT * FROM `applicant_status` WHERE `application_no` = '".$label."' LIMIT 1";
