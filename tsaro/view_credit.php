						...
						<?php
							...
							while ($row = $database->fetch_array($result)) {
								if($serialno % $max == 1){
									...
									echo '<form action="edit_credit.php" method="POST">
									<tr>
										<td>'.$serialno.'</td>
										<td>'.$row["fullname"].'</td>
										<td>'.$row["email"].'</td>';
										
										echo '<td><button type="submit" class="btn btn-info" name="edit_credit_submit">Edit</button></td>
										
									</tr>
									<input type="hidden" name="cid" value="'.customEncrypt($row["credit_id"]).'" />
									...
									elseif($serialno == $number_of_credits){
										echo '</tbody>';
										echo '</table>';
