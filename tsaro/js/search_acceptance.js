...
jQuery(document).ready(function(){
	...
	jQuery('#search_acceptance_button').click(function(e){
		e.preventDefault();
		jQuery('.pagination').hide();
		jQuery('#view_div').html('Loading...');
		jQuery.ajax({
			type:'POST',
			url:'ajax/ajax_search_acceptance.php',
			data: jQuery(".search_acceptance").serialize()	
		}).done(function(value){
			currentpage = 1;
	...
	jQuery('#search_acc_acceptance_button').click(function(e){
		e.preventDefault();
		jQuery('.pagination').hide();
		jQuery('#view_div').html('Loading...');
		jQuery.ajax({
			type:'POST',
			url:'ajax/ajax_search_acc_acceptance.php',
			data: jQuery(".search_acceptance").serialize()	
		}).done(function(value){
			currentpage = 1;
			var value = value.split('pagenumber=');
